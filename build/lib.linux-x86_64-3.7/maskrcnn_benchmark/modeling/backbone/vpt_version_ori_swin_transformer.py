...
class PromptedSwinTransformer(SwinTransformer):
    def __init__(
        ...
        self.out_features=('stage2', 'stage3', 'stage4', 'stage5')
        # for i_layer in range(self.num_layers):
    ...
    def forward_features(self, x):
        ...
        if self.prompt_config.LOCATION == "prepend" and self.prompt_config.DEEP:
            ...
            if not GLIPVPT:
                ...
                outs = []
                SWITCH=[384,768,1536,3072]
                outs.append(x[:, :-100, :].view(-1, 200, 200, 192).permute(0, 3, 1, 2))
                for i in range(self.num_layers):
                    ...
                    if i<=2:
                        x_unprep = x[:, :-100, :]
                        wide=200//(2**(i+1))
                        out = x_unprep.view(-1, wide,wide, SWITCH[i]).permute(0, 3, 1, 2)
                        outs.append(out)
                return outs
        x = self.norm(x)  # B L C
        x = self.avgpool(x.transpose(1, 2))  # B C 1
