...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.,adap_mlp=0):
        ...
        if adap_mlp<0:
            ...
            print('lora equiped~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
            self.lora_qkv = lora.Linear(dim,dim*3,bias=qkv_bias,r=16)
        else:
    ...
    def forward(self, x, mask=None):
        ...
        B_, N, C = x.shape
        if self.adap_mlp<0:
            qkv = self.lora_qkv(x).reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        else:
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=7, shift_size=0,
        ...
        self.attn = WindowAttention(
            ...
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop,adap_mlp=adap_mlp)
...
class SwinTransformer(nn.Module):
        ...
        if cfg.use_upt:
            ...
            class UPT(nn.Module):
                def __init__(self, input_size, hidden_size):
                    ...
                    self.sa = nn.MultiheadAttention(hidden_size, num_heads=1)
    ...
    def incorporate_prompt(self, x):
        ...
        if self.prompt_config.LOCATION == "prepend":
            ...
            prompt_embd = self.prompt_dropout(
                self.prompt_embeddings.expand(B, -1, -1)).permute(0,2,1)
            x = x.view((1, 192, -1))
            x = torch.cat((
                    prompt_embd, x[:,:,self.cfg.MODEL.PROMPT.NUM_TOKENS:]
                ), dim=2)
            # (batch_size, n_prompt + n_patches, hidden_dim)
            x=x.view(-1,192,200,200)
        elif self.prompt_config.LOCATION == "add":
            # add to the input patches + CLS
            # assert self.prompt_config.NUM_TOKENS == x.shape[1]
            x=torch.cat((x,self.prompt_embeddings),dim=1)
            x = self.get_patch_embeddings(x) # (batch_size, 1 + n_patches, hidden_dim)
            # x=x.view(1,192,-1)
            # x += self.prompt_dropout(
            #     self.prompt_embeddings.transpose(1,2))
            # x=x.view(1,192,200,200)
    ...
    def forward(self, x):
        ...
        if self.cfg.use_maple:
            x,maple_project_out=x
        if self.cfg.vpt_only==-1:
            x = self.incorporate_prompt(x)
        else:
            x = self.patch_embed(x)
        if self.cfg.use_upt:
            seed=torch.randn((1,1,192)).cuda()
            UPT_OUTPUT_of_seed=self.UPT(seed)
            x[:, :192, :1, :1] += UPT_OUTPUT_of_seed.view(-1,192,1,1)
        if self.cfg.use_maple:
            x[:, :192, :1, :1] += maple_project_out
        Wh, Ww = x.size(2), x.size(3)
        if self.ape:
...
def build_swint_backbone(cfg):
    ...
    """
    import time
    if cfg.fuse_module_cross_att%10>=2 or cfg.vpt_only > 0:
        print('building adapter-equipped and vpt-equipped swint!::::::::::::::::::::::::::::::::::::::::::::::::::::::!')
        from .vpt_version_ori_swin_transformer import PromptedSwinTransformer
        return PromptedSwinTransformer(cfg.MODEL.PROMPT,img_size=800, patch_size=4, in_chans=3,
                                       # num_classes=-1,
        ...
        return SwinTransformer(cfg,
            ...
            adap_mlp=cfg.SWINBLO-10*cfg.use_lora_visual
        )